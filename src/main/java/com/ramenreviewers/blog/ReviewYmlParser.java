...
public class ReviewYmlParser {
    ...
    public static Review createReview(Map<String, Object> propertyMap) {
        ...
        var scoreBroth = getFloatScoreValue(propertyMap, "scoreBroth", Review.MIN_SCORE, Review.MAX_SCORE_BROTH);
        var scoreNoodles = getFloatScoreValue(propertyMap, "scoreNoodles", Review.MIN_SCORE, Review.MAX_SCORE_BROTH);
        var scoreToppings = getFloatScoreValue(propertyMap, "scoreToppings", Review.MIN_SCORE, Review.MAX_SCORE_TOPPINGS);
        var scoreAtmosphere = getFloatScoreValue(propertyMap, "scoreAtmosphere", Review.MIN_SCORE, Review.MAX_SCORE_ATMOSPHERE);
        ...
        System.out.println(String.format("%f | %f | %f | %f", scoreBroth, scoreNoodles, scoreToppings, scoreAtmosphere));
    ...
    private static float getFloatScoreValue(Map<String, Object> propertyMap, String key, int minScore, int maxScore) {
        return propertyMap.containsKey(key) ? Math.clamp((int) propertyMap.get(key), minScore, maxScore) : -1;
    }

    private static float calculateTotalScore(float scoreBroth, float scoreNoodles, float scoreToppings, float scoreAtmosphere) {
        return Review.MAX_TOTAL_SCORE * (scoreBroth + scoreNoodles + scoreToppings + scoreAtmosphere)
